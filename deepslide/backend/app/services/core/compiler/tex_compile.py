...
def compile_content(base_dir: str) -> Dict[str, Any]:
    ...
    # We will need to locate the run_in_docker.sh script
    # It was originally in deepslide/version-20260120/run_in_docker.sh
    # We should probably copy it to deepslide-v3/backend/run_in_docker.sh or similar
    # For now, let's assume it is in the same directory as this file
    run_script = os.path.join(os.path.dirname(__file__), "run_in_docker.sh")

    if not os.path.exists(run_script):
        # Fallback to backend root if not found here
        backend_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../"))
        run_script_alt = os.path.join(backend_root, "run_in_docker.sh")
        if os.path.exists(run_script_alt):
            run_script = run_script_alt
        else:
