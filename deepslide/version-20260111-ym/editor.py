...
def render_page_ppt_editor(tmp_base):
    ...
    with c_l:
        ...
        if pages:
            ...
            _, nav_prev, _, nav_star, _, nav_next, _ = st.columns([2.2, 0.9, 2.2, 0.9, 2.2, 0.9, 2.2])
            ...
            # with nav_page:
            #     st.markdown(
            #         f"<div style='text-align:center; font-size:12px; color:rgba(30,41,59,0.78); padding-top:6px;'>{curr+1} / {len(pages)}</div>",
            #         unsafe_allow_html=True,
            #     )
