...
TEMPLATES: Dict[str, NarrativeTemplate] = {
    ...
    "heros_journey": NarrativeTemplate(
        ...
        constraints=[
            ...
            "必须出现 FalseLead（一次错误直觉/无效尝试/被排除路线）。注意：FalseLead 若原文没写，必须标为 bridge/rhetorical，不能冒充 paper 事实。",
    ...
    "pipeline": NarrativeTemplate(
        ...
        constraints=[
            "尽量保持原文叙述顺序，不要重排太多。",
            "SystemOrAlgorithm 节点应概括核心方法/框架，Experiments 必须来自原文。",
            "结尾 Conclusion 对整体做收束，并指出意义/局限/未来方向。",
        ],
        edge_hints=["铺垫（背景说明）", "提出问题（导致…）", "方案（我们提出…）", "验证（实验表明…）", "总结（因此…）"],
        requirements=TemplateRequirements(
            first_roles=["Background", "Problem"],
    ...
    "storytelling": NarrativeTemplate(
        template_id="storytelling",
        title_cn="讲故事式（Storytelling）",
        one_liner="以人物/场景驱动：动机—冲突—探索—转折—解决—余波。",
        roles=["Protagonist", "Motivation", "Conflict", "Investigation", "Twist", "Resolution", "Aftermath"],
        constraints=[
            "必须有 Conflict（矛盾/痛点），并描述为具体场景或矛盾。",
            "Twist 是一个重要的反转/新洞见，最好与原文关键方法或发现对齐。",
            "Aftermath 解释解决方案带来的后续影响/意义。",
        ],
        edge_hints=["动机（因为…）", "冲突（但遇到…）", "探索（我们尝试…）", "转折（关键发现…）", "解决（最终…）", "余波（这意味着…）"],
        requirements=TemplateRequirements(
            first_roles=["Protagonist", "Motivation"],
            role_min_counts={"Conflict": 1, "Investigation": 1, "Twist": 1, "Resolution": 1},
            order_constraints=[("Conflict", "Investigation"), ("Investigation", "Twist"), ("Twist", "Resolution")],
        ),
    ),
    "teaser_then_depth": NarrativeTemplate(
        template_id="teaser_then_depth",
        title_cn="先吊胃口再深入（Teaser → Depth）",
        one_liner="先抛亮点/对比/结果 teaser，引出好奇，再分层讲细节。",
        roles=["Teaser", "WhyInteresting", "Setup", "MethodLayer1", "MethodLayer2", "KeyResult", "Takeaway"],
        constraints=[
            "Teaser 必须来自原文的亮点或对比（paper+证据）。",
            "MethodLayer1/2 分层讲解：先大框架，再关键细节。",
            "KeyResult 必须来自原文，带 evidence。",
        ],
        edge_hints=["吸引（亮点是…）", "解释（为什么有趣）", "铺垫（背景/设定）", "分层展开（先…再…）", "结果（实验/对比表明…）", "启示（因此…）"],
        requirements=TemplateRequirements(
            first_roles=["Teaser"],
            role_min_counts={"MethodLayer1": 1, "MethodLayer2": 1, "KeyResult": 1, "Takeaway": 1},
            order_constraints=[("Teaser", "MethodLayer1"), ("MethodLayer1", "MethodLayer2"), ("MethodLayer2", "KeyResult")],
        ),
    ),
}
