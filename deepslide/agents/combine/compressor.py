...
from deepslide.utils.chapter_node import ChapterNode
...
class Compressor:
    ...
    def compress(self, logic_flow: LogicFlow, nodes: List[ChapterNode], image_list: List[str] = None) -> Tuple[Content, List[Spection]]:
        """
...
"""
        ...
        def add_slide(latex_body: str, speech_script: str):
            ...
            if self.current_duration > target_duration_sec:
                #  msg += " WARNING: Time budget exceeded! Please wrap up immediately and reply DONE."
                msg += " WARNING: Time budget exceeded! Please ensure the content is complete and conclude as soon as possible."
            return msg
...
3. **Use Images**: Use `list_available_images` to see what images are available.
   ...
   - **Constraint**: When including images or tables, ALWAYS ensure they fit within the slide frame. Use `[width=0.8\textwidth,height=0.7\textheight,keepaspectratio]` for images. For tables, consider using `\resizebox{{0.9\textwidth}}{{!}}{{...}}` or `\footnotesize`.
   - **Constraint**: If a slide contains an image, reduce the amount of text. Use only 1-2 brief bullet points or a short caption. DO NOT clutter the slide.
   - **Constraint**: Do NOT truncate speech scripts. Generate COMPLETE sentences. Do not use "..." unless it is a quote.
   - **Constraint**: Limit text density. Each slide should generally have 3-5 bullet points maximum. If an image is present, limit to 1-2 points.
4. **Generate Slides**: Use `add_slide` to create slides.
...
5. **Correspondence**: `add_slide` adds one frame and one speech script. Do NOT add frames without speech or vice versa.
6. **Structure**: Use `add_section` for logical separation if needed.
