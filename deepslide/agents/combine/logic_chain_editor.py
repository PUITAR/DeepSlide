...
try:
    ...
    try:
        ...
        from deepslide.agents.transformer.data_types import LogicNode, LogicFlow
...
class LogicChainUI:
    ...
    def render(self) -> None:
        st.subheader("Logic Chain Editor")
        for i, node in enumerate(self.editor.nodes):
            with st.expander(f"Node {i+1}"):
                with st.form(f"form_node_{i}", clear_on_submit=False):
                    name = st.text_input("Name", node.name, key=f"name_{i}")
                    desc = st.text_area("Description", node.description, key=f"desc_{i}")
                    dur = st.text_input("Duration", node.duration, key=f"dur_{i}")
                ...
                with cols[0]:
                    if st.button("Move Up", key=f"up_{i}") and i > 0:
                        ...
                        st.rerun()
                with cols[1]:
                    if st.button("Move Down", key=f"down_{i}") and i < len(self.editor.nodes) - 1:
                        self.editor.move_node(i, i + 1)
                        st.rerun()
                with cols[2]:
        ...
        with st.expander("Edit Reference Edges", expanded=False):
            ...
            for j, e in enumerate(self.editor.edges):
                ...
                with c1:
                    ...
                    st.text(f"From: {self.editor.nodes[int(e['from'])].name} ({e['from']})")
                with c2:
                    st.text(f"To: {self.editor.nodes[int(e['to'])].name} ({e['to']})")
                with c3:
                    reason = st.text_input(f"Reason {j}", e.get("reason", ""), key=f"ref_reason_{j}")
                with c4:
                    if st.button("Delete", key=f"ref_del_{j}"):
        ...
        st.subheader("Logic Chain Visualization")
