...
class Compiler:
    ...
    def __init__(self, max_try: int = 3):
        ...
        if os.path.exists(usr_prompt_path):
            self.usr_prompt = open(usr_prompt_path, 'r', encoding='utf-8').read()
            print(f"DEBUG: Loaded usr_prompt from {usr_prompt_path}")
        else:
            self.usr_prompt = "Error: File {0} with message: {1}\nOriginal content:\n{2}\nPlease fix it."
            logger.warning(f"User prompt file not found at {usr_prompt_path}, using default.")
            print("DEBUG: Used default usr_prompt")
        ...
        sys_msg = BaseMessage.make_assistant_message(
            ...
            content= open(
                ...
                os.path.join(os.path.dirname(__file__), 'prompts/sys_prompt_compiler.md'),
                'r',
                encoding='utf-8'
            ).read()
        )
...
4. If you fix it, reply with "Fix: [explanation]".
"""
        ...
        for _ in range(8): # Give it more steps
             ...
             if "Fix:" in str(response.msg.content):
                 print(Fore.YELLOW + f"🔧 Agent Analysis\n {response.msg.content}\n" + Fore.RESET)
                 return True
             if "Update success" in str(response.info) or "Write success" in str(response.info):
                  # If tool executed successfully, assume fix attempted
                  print(Fore.YELLOW + f"🔧 Agent used tool to fix.\n" + Fore.RESET)
                  return True
