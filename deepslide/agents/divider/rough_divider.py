...
from chapter_node import ChapterNode, NodeType, PositionInfo, ContentMetrics
...
class RoughDivider:
    ...
    def _parse_instructions(self, instructions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 验证 max_section_depth
        max_depth = instructions.get("max_section_depth", 3)
        if not isinstance(max_depth, int) or max_depth < 1 or max_depth > 5:
            logger.warning(f"无效 max_section_depth: {max_depth}，回退到 3")
            max_depth = 3
        
        # 验证 max_sections
        max_sections = instructions.get("max_sections", 15)
        if not isinstance(max_sections, int) or max_sections < 1 or max_sections > 50:
            logger.warning(f"无效 max_sections: {max_sections}，回退到 15")
            max_sections = 15
        
        # 验证 merge_short_threshold
        merge_threshold = instructions.get("merge_short_threshold", 150)
        if not isinstance(merge_threshold, int) or merge_threshold < 10 or merge_threshold > 500:
            logger.warning(f"无效 merge_short_threshold: {merge_threshold}，回退到 150")
            merge_threshold = 150
    ...
    def _apply_merge_strategy(
        ...
        """
        应用合并策略 - 永不丢弃内容
        1. 为每个 section 添加 has_academic_content 缓存
        2. 修正跳过关键词逻辑：当有学术内容时，即使包含跳过关键词也不合并
        3. 修正位置信息计算：精确计算合并后内容的实际结束位置
        """
        merged = []
        merge_log = []
