...
def parse_vlm_response(response_text):
    """Extract LaTeX code from VLM response."""
...
def process_slide_modification(instruction):
    ...
    try:
        ...
        new_frame = parse_vlm_response(resp)
        if new_frame:
            ...
            try:
                ...
                if compile_success:
                    try:
                        ...
                        # Read content.tex for alignment
                        content_tex_path = os.path.join(output_dir, "content.tex")
                        content_tex = ""
                        if os.path.exists(content_tex_path):
                            with open(content_tex_path, "r") as f:
                                content_tex = f.read()
                        
                        if os.path.exists(pdf):
                            print("Start alignment...")
                            # Pass content_tex to alignment
                            alignment_result = aligner.align(pdf, [str(s) for s in speeches], content_tex=content_tex)
                            
                            # Update speeches with aligned result
                            aligned_speeches = alignment_result.get("speeches", [])
                            if aligned_speeches:
                                speeches = aligned_speeches
                                print(f"Alignment completed. {len(speeches)} speeches.")
                            
                            # Save alignment metadata for graph generation
                            alignment_data = alignment_result.get("alignment", [])
                            if alignment_data:
                                align_json_path = os.path.join(output_dir, "alignment_meta.json")
                                with open(align_json_path, "w") as f:
                                    json.dump(alignment_data, f, ensure_ascii=False, indent=2)
                                print(f"Alignment metadata saved to {align_json_path}")
                ...
                if compile_success and os.path.exists(st.session_state.generated_files["pdf"]):
                    ...
                    try:
                        ...
                        # Don't crash if preview fails
                        st.session_state.preview_state["pdf_pages"] = []
        ...
        if st.button("Generate Slide Graph"):
            with st.spinner("Generating Slide Graph..."):
                try:
                    ...
                    # Check if alignment metadata exists
                    align_meta_path = os.path.join(files["audio_dir"], "alignment_meta.json")
                    alignment_data = None
                    if os.path.exists(align_meta_path):
                        with open(align_meta_path, "r") as f:
                            alignment_data = json.load(f)
                            print(f"Loaded {len(alignment_data)} alignment pairs from metadata.")
                    
                    # Call run (if alignment_data provided, it should use it instead of manual pairing)
                    # We need to update SlideGraphGenerator.run to accept alignment_data
                    # But for now, we can modify app.py to pass it if the generator supports it, 
                    # OR we can let the generator construct it.
                    # Wait, the user requirement was: "slide_graph_generator...不用自己去找二元组了，而是使用上面生成的就可以了"
                    # So we MUST pass this alignment_data to the generator.
                    
                    res = gen.run(content_tex, speeches, logic_chain, use_llm=True, precomputed_alignment=alignment_data)
    ...
    with bottom_box:
        ...
        with c_btn:
            ...
            if st.button("💾", help="Save as Cloned Voice", key="ed_btn_save_clone"):
                if "last_ed_audio_path" in st.session_state:
                        st.session_state.cloned_voice_path = st.session_state.last_ed_audio_path
                        st.session_state.selected_voice_path = st.session_state.last_ed_audio_path
                        st.toast("✅ Voice saved as Cloned Voice!")
                else:
                        st.toast("⚠️ No audio recorded to save.")
        ...
        if submitted and user_text:
            ...
            with st.spinner("Modifying..."):
                process_slide_modification(user_text)
            st.rerun()
