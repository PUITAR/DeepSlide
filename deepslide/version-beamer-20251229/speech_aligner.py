...
class SpeechAligner:
    ...
    def _extract_text_subprocess(self, pdf_path: str) -> List[str]:
        """Extract text from PDF using a subprocess to avoid segfaults in main thread."""
        script_content = """
import fitz
import json
import sys
...
try:
    ...
    for i, page in enumerate(doc):
        ...
        pages.append(f"Page {i+1}: {text[:800]}")
    doc.close()
...
"""
        ...
        try:
            # Run the script with the PDF path as argument
            result = subprocess.run(
                [sys.executable, tmp_path, pdf_path], 
                capture_output=True, 
                text=True, 
                check=True
            )
            ...
            if isinstance(data, dict) and "error" in data:
                logger.error(f"Subprocess PDF error: {data['error']}")
                return []
    ...
    def align(self, pdf_path: str, original_speeches: List[str]) -> List[str]:
        ...
        # 1. Extract text from PDF (using subprocess)
        pdf_pages_text = self._extract_text_subprocess(pdf_path)
