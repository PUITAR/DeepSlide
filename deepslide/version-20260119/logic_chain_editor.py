...
class LogicChainUI:
    ...
    def _handle_event(self, event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            elif cmd == "delete_node" and idx_raw is not None:
                idx = int(idx_raw)
                if 0 <= idx < len(self.editor.nodes):
                    self.editor.remove_node(idx)
                    st.toast("Node deleted")
                handled = True
...
def _prepare_board_data(
    ...
    node_html = "".join(
        [
            ...
            + "<button class='ds-act-btn ds-delete-btn' title='Delete Node'>🗑</button>"
            + "</div>"
