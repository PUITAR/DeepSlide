...
def render_page_ppt_editor(tmp_base):
    ...
    with c_r:
        ...
        with tabs[1]:
            ...
            if clone_audio:
                ...
                if st.button("Save Cloned Voice", icon=":material/save:", use_container_width=True):
        ...
        with tabs[2]:
            ...
            #  st.markdown("The AI will analyze the slide image and LaTeX code, then optimize the layout and styling iteratively.")
             ...
             if st.button("Run Optimization", icon=":material/auto_fix_high:", type="primary", use_container_width=True):
        ...
        with tabs[3]:
            ...
            if st.button("Enrich References", icon=":material/hub:", type="primary", use_container_width=True):
            ...
            if st.button("Generate HTML", icon=":material/html:", type="primary", use_container_width=True):
    ...
    c_a1, c_a2 = st.columns([1, 1], gap="medium")
    with c_a1:
        if st.button("Update Changes", icon=":material/refresh:", use_container_width=True):
            ...
            if c.run(os.path.dirname(files["tex"])).get("success"):
                ...
                st.rerun()
    with c_a2:
        if st.button("Finish & Export", icon=":material/check_circle:", type="primary", use_container_width=True):
            with open(files["tex"], "w") as f: f.write(n_tex)
            with open(files["speech"], "w") as f: f.write(n_sp)
            if n_title:
                with open(title_path, "w") as f: f.write(n_title)
    ...
    # Inject custom CSS for this page's bottom section (Copied & Adapted from requirements.py)
    st.markdown("""
        <style>
    ...
    with bottom_box:
        ...
        c_audio, c_transcribe, c_btn = st.columns([12, 0.8, 0.8], gap="small", vertical_alignment="center")
        with c_audio:
            audio_value = st.audio_input("Record Command", key="ed_audio", label_visibility="collapsed")
        with c_transcribe:
            if st.button("", icon=":material/description:", help="Transcribe to Text", key="ed_btn_transcribe"):
        ...
        with c_btn:
            ...
            if st.button("", icon=":material/save:", help="Save as Cloned Voice", key="ed_btn_save_clone"):
