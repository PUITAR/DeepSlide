...
def _tool_generate_data_visualization(latex_table_code):
    ...
    **Return ONLY the HTML code (no markdown backticks).**
    ...
    try:
        ...
        clean = resp.strip()
        if clean.startswith("```html"): clean = clean[7:]
        if clean.startswith("```"): clean = clean[3:]
        if clean.endswith("```"): clean = clean[:-3]
        return clean.strip()
    except Exception as e:
        _log(f"Viz Tool Error: {e}")
        return f"<div class='error'>Visualization Failed</div>"
...
def generate_html_via_llm_iterative(tex_content, speeches, pdf_pages):
    ...
    # --- Title Page (Page 0) ---
    if len(speeches) > 0 and len(pdf_pages) > 0:
        # Title page usually doesn't have a \begin{frame} in content.tex, it's in title.tex or auto-generated
        # We can just generate a simple Title Section
        # Or ask LLM to generate it from title.tex if we load it
        title_speech = speeches[0].replace("<add>", "").strip()
        
        # We can use the image background for fidelity, and overlay simple text if needed
        # Or just rely on the image background entirely for visual fidelity
        # The user said "pdfbeamer version slide and speech one-to-one copy"
        # Using the PDF image as background is the SAFEST way to ensure "one-to-one" visual match.
        # But if we want HTML *text* (editable/selectable), we need conversion.
        # The user complained "generated html slide and pdf do not match".
        # This implies the previous "one-shot" LLM generation failed to replicate the PDF layout/content.
        
        # Strategy: 
        # Hybrid Approach: Use LLM to generate HTML structure (text), but inject the PDF image as background?
        # OR just improve the text conversion.
        
        # Let's convert text structure for "semantic" HTML, but maybe rely on CSS/Classes better.
        
        # Let's convert Title Page
        # We can try to read title.tex
        try:
            title_path = os.path.join(os.path.dirname(st.session_state.generated_files["tex"]), "title.tex")
            if os.path.exists(title_path):
                with open(title_path, "r") as f: title_tex = f.read()
                # Wrap in frame for consistency
                title_frame = f"\\begin{{frame}}\n{title_tex}\n\\end{{frame}}"
                html_sec = _convert_single_slide_to_html(title_frame, title_speech)
                # Resolve images
                html_sec = _resolve_image_paths_in_html(html_sec, base_dir)
                full_html_sections.append(html_sec)
            else:
                # Fallback
                full_html_sections.append(f"<section><h1>Title Page</h1><aside class='notes'>{title_speech}</aside></section>")
        except:
             full_html_sections.append(f"<section><h1>Title Page</h1><aside class='notes'>{title_speech}</aside></section>")
    ...
    for i in range(1, len(speeches)):
        ...
        if current_frame_idx < len(matches):
            ...
            # Process placeholders (Tool Calling)
            html_sec = _process_html_placeholders(html_sec, base_dir)
            
            # Resolve images (Fallback for any missed ones)
            html_sec = _resolve_image_paths_in_html(html_sec, base_dir)
            
            # Inject Background Image (PDF Page) for visual fidelity?
            # User said "generated html slide and pdf do not match".
            # If we purely use text, it's hard to match PDF layout (columns, images, positioning).
            # The BEST way to match PDF is to use the PDF image as background.
            # AND have the HTML text hidden or overlaid?
            # Or maybe the user WANTS editable HTML text?
            # "one-to-one copy" implies content match.
            
            # Let's try to inject the PDF image as `data-background-image`
            # AND keep the converted HTML content on top?
            # Or maybe just use the HTML content and try to make it look good?
            
            # Let's inject the PDF image as background, but add a class to possibly hide it or the text?
            # If we show BOTH, they will overlap messily.
            
            # IMPROVEMENT:
            # Let's trust the LLM conversion for CONTENT, but use the PDF image as a REFERENCE or Fallback?
            # Or: The previous "One-click" (Agent) method generated a single file and likely hallucinated or missed structure.
            # Iterative conversion is safer.
            
            # Let's add the image as background but maybe lower opacity?
            # No, standard Reveal.js practice:
            # Option A: Image Background + Hidden Text (Notes only). (Perfect visual match, but not "HTML" content)
            # Option B: Reconstructed HTML. (Good for accessibility/editing, bad for 1:1 visual match)
            
            # The user complained "don't match". 
            # I will use the PDF image as the background for the slide to ensure visual fidelity,
            # and put the LLM-converted HTML content inside.
            # Users can then choose to hide the HTML content via CSS if they just want the PDF look,
            # or hide the background if they want pure HTML.
            # I'll default to showing the HTML content on top of a subtle or no background?
            # Let's stick to the prompt: "Convert... into HTML".
            # I will inject the PDF image as `data-background-image` so it is ALWAYS there.
            # And I will style the HTML content to be visible.
            
            if i < len(pdf_pages):
                img_path_or_bytes = pdf_pages[i]
                
                # Check if it is a path (str) or bytes
                if isinstance(img_path_or_bytes, str):
                    if os.path.exists(img_path_or_bytes):
                        with open(img_path_or_bytes, "rb") as f:
                            img_bytes = f.read()
                    else:
                        img_bytes = b"" # File missing
                else:
                    img_bytes = img_path_or_bytes # Assume it is bytes
                
                if img_bytes:
                    b64_img = base64.b64encode(img_bytes).decode('utf-8')
                    img_src = f"data:image/png;base64,{b64_img}"
                    # Inject into <section> tag. Use no-repeat and center to fix path/display issues.
                    # Also use 100% size to fit container.
                    html_sec = html_sec.replace("<section>", f'<section data-background-image="{img_src}" data-background-size="100% 100%" data-background-opacity="0.1" data-background-repeat="no-repeat" data-background-position="center">')
                else:
                    # Fallback if image bytes are missing
                    html_sec = html_sec.replace("<section>", f'<section data-background-color="#f0f0f0">')

                # Note: opacity 0.1 allows text to be readable while showing "ghost" of original?
                # Or maybe 1.0 if we rely on image?
                # Let's use 1.0 (Full Image) and put text in a hidden/overlay div?
                # "preview时同步上下页翻转" implies visual checking.
                
                # If I use 1.0 opacity, the HTML text will overlay the image text -> Ugly.
                # If I use 0 opacity, it's just HTML.
                
                # Decision: The user wants "HTML version slide". This usually means REAL HTML text.
                # The "don't match" likely refers to missing content or wrong order.
                # Iterative conversion fixes the order/missing content.
                # I will NOT force the background image to full opacity to avoid clash.
                # I will use it as a reference or keep it simple.
                # Let's just use the converted HTML.
                pass
            ...
            full_html_sections.append(f"<section><h3>(Slide {i+1})</h3><aside class='notes'>{speech}</aside></section>")
	...
	<body>
		...
		<script>
			Reveal.initialize({{
                ...
                width: 1280, height: 720, margin: 0.1, minScale: 0.2, maxScale: 2.0,
				plugins: [ RevealNotes ]
			}}).then(() => {{
        ...
        with tabs[6]:
            ...
            if "html_slides_content" in st.session_state:
                with st.expander("HTML Slide Preview", expanded=True):
                    ...
                    components.html(synced_html, height=600, scrolling=True)
