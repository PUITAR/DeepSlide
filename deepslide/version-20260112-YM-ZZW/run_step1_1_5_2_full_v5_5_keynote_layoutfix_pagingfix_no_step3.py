...
# Step1 inputs
BEAMER_TXT_PATH = r"/home/ym/DeepSlide/deepslide/version-20260112-YM-ZZW/content2.txt"     # "/abs/path/to/beamer.txt"
PDF_PATH        = r"/home/ym/DeepSlide/deepslide/version-20260112-YM-ZZW/base2.pdf"     # "/abs/path/to/rendered.pdf"

# Prompts / templates (assume this .py and html_diedai/ are in the same directory)
STEP1_PROMPT_PATH = str(BASE_DIR / "html_diedai" / "step1_prompt_beamertext_v9_keynote_mathlayout_table.txt")
STEP1_TEMPLATE_PATH = str(BASE_DIR / "html_diedai" / "step1_template_step1_beamertext_v8_tables.html")

STEP1_5_PROMPT_PATH = str(BASE_DIR / "html_diedai" / "step1_5_planner_prompt_v16_tables_keynote_layout.json.txt")

# --- Step2 prompts: dynamic shell + per-slide (avoid truncation)
STEP2_SHELL_PROMPT_PATH = str(BASE_DIR / "html_diedai" / "step2_shell_prompt_v6_css_only_keynote_layout_tables.txt")
STEP2_SLIDE_PROMPT_PATH = str(BASE_DIR / "html_diedai" / "step2_slide_prompt_v4_dynamic_per_slide_keynote_layout_tables.txt")

# ---- Step3 (polish) ----
STEP3_ENABLE = True
STEP3_PROMPT_PATH = os.path.join(BASE_DIR, "html_diedai", "step3_polish_prompt_v1_per_slide_minimal.txt")
STEP3_OUT_HTML = "step3_visual_deck.html"
...
"""
    ...
    paging_css = r"""
/* =================== SYSTEM PAGING OVERRIDE ===================
   Force slide-by-slide (no long scroll). Only the .is-active slide is visible.
   This block is appended by the pipeline to prevent LLM CSS from breaking paging.
*/
html, body { height: 100%; margin: 0; overflow: hidden !important; }
.deck-root { height: 100%; overflow: hidden !important; }
.stage { position: relative !important; width: 100%; height: 100%; overflow: hidden !important; }
section.slide {
...
def _extract_step2_slide_sections(deck_html: str):
    """Return list of (slide_id, full_section_html)."""
    # Step2 slide sections are generated as <section ... data-slide-id="sX" ...> ... </section>
    out = []
    for m in re.finditer(r'(<section\b[^>]*\bdata-slide-id\s*=\s*"([^"]+)"[^>]*>[\s\S]*?</section>)', deck_html, flags=re.I):
        out.append((m.group(2), m.group(1)))
    return out


def _heuristic_overflow_risk(section_html: str) -> bool:
    """Conservative heuristic: flag slides likely to overflow (many bullets, long text, tables)."""
    li_cnt = len(re.findall(r'<li\b', section_html, flags=re.I))
    table_cnt = len(re.findall(r'<table\b', section_html, flags=re.I))
    # rough visible text length (strip tags)
    txt = re.sub(r'<[^>]+>', ' ', section_html)
    txt = re.sub(r'\s+', ' ', txt).strip()
    tlen = len(txt)
    # conservative thresholds
    return (li_cnt >= 9) or (table_cnt >= 1 and tlen >= 450) or (tlen >= 1200)


def _ensure_slide_data_fit(section_html: str, min_px: int = 14) -> str:
    """If slide root lacks data-fit, add it so the runtime fitter can shrink text if needed."""
    # Only touch opening <section ...> tag
    m = re.search(r'^\s*<section\b([^>]*)>', section_html, flags=re.I)
    if not m:
        return section_html
    opening = m.group(0)
    attrs = m.group(1)
    if re.search(r'\bdata-fit\b', attrs):
        return section_html
    # Insert data-fit and min limit
    new_opening = opening[:-1] + f' data-fit data-fit-min="{min_px}">' 
    return new_opening + section_html[m.end(0):]


def run_step3(step2_deck_path: Path, out_dir: Path, step3_prompt_path: Path) -> Path:
    """Per-slide polish pass on Step2 deck HTML to fix minor formatting/overflow issues.

    This runs LLM *per slide* to avoid truncation, and only applies minimal changes.
    """
    from ppt_requirements_collector import CamelAIClient
    camel = CamelAIClient()

    deck_html = read_text(str(step2_deck_path))
    slides = _extract_step2_slide_sections(deck_html)
    if not slides:
        print('[STEP3][WARN] No step2 slide <section ... data-slide-id=...> found; skip polish.')
        return step2_deck_path

    step3_prompt = read_text(str(step3_prompt_path))

    polished_map = {}
    for idx, (sid, sec) in enumerate(slides):
        # lightweight deterministic pre-fix (only if likely to overflow)
        sec_in = sec
        if _heuristic_overflow_risk(sec_in):
            sec_in = _ensure_slide_data_fit(sec_in, min_px=14)

        user_input = (
            f"SLIDE_INDEX={idx+1}/{len(slides)}\n"
            f"SLIDE_ID={sid}\n"
            "\n"
            "You will be given ONE slide section. Polish it minimally.\n"
            "Return ONLY the updated <section ...>...</section>.\n"
            "\n"
            "=== SLIDE_HTML_BEGIN ===\n" + sec_in + "\n=== SLIDE_HTML_END ===\n"
        )
        resp = camel.get_response(user_input, system_prompt=step3_prompt)
        resp2 = strip_code_fences(resp).strip()
        sm = re.search(r'<section\b[\s\S]*?</section>', resp2, flags=re.I)
        if not sm:
            # If model fails, keep preprocessed original
            dbg = out_dir / f"step3_slide_{sid}.raw.txt"
            dbg.write_text(resp2, encoding='utf-8')
            polished_map[sid] = sec_in
            print(f"[STEP3][WARN] slide polish parse failed for {sid}; kept original (see {dbg}).")
            continue
        polished = postprocess_html(sm.group(0).strip())
        polished_map[sid] = polished
        print(f"[STEP3] slide {idx+1}/{len(slides)} ok: {sid}")

    # Rebuild deck HTML by replacing each slide section in order
    out_parts = []
    last = 0
    for m in re.finditer(r'(<section\b[^>]*\bdata-slide-id\s*=\s*"([^"]+)"[^>]*>[\s\S]*?</section>)', deck_html, flags=re.I):
        out_parts.append(deck_html[last:m.start(1)])
        sid = m.group(2)
        out_parts.append(polished_map.get(sid, m.group(1)))
        last = m.end(1)
    out_parts.append(deck_html[last:])

    final_html = postprocess_html(''.join(out_parts))
    out_path = out_dir / STEP3_OUT_HTML
    out_path.write_text(final_html, encoding='utf-8')
...
def main():
    ...
    # Step2
    step2_out_path = run_step2(step1_html_with_imgs, plan_json_path, out_dir, step2_shell_prompt, step2_slide_prompt)

    # Step3: per-slide polish (optional)
    if STEP3_ENABLE:
        step3_prompt = Path(STEP3_PROMPT_PATH)
        if not step3_prompt.is_absolute():
            step3_prompt = Path(BASE_DIR) / 'html_diedai' / step3_prompt.name
        if step3_prompt.exists():
            run_step3(step2_out_path, out_dir, step3_prompt)
        else:
            print('[STEP3][WARN] prompt not found: ' + str(step3_prompt) + ' (skip)')
