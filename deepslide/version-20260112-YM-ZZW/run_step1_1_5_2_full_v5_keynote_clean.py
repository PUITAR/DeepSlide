...
STEP2_PROMPT_PATH   = str(BASE_DIR / "html_diedai" / "step2_executor_prompt_v13_images_keynote.txt")
...
def run_step1_5(step1_html_path: Path, out_dir: Path, step1_5_prompt: str) -> Path:
    ...
    if obj is None:
        print("[STEP1.5][WARN] JSON parse failed, trying LLM repair ...")
        fixed = repair_json_with_llm(camel, resp)
        obj = extract_json_obj(fixed)
        if obj is None:
            raise RuntimeError("Step1.5 failed to produce valid JSON after repair.")
        resp = json.dumps(obj, ensure_ascii=False, indent=2)

    json_path = out_dir / STEP1_5_OUT_JSON
    write_text(json_path, json.dumps(obj, ensure_ascii=False, indent=2))
    print(f"[STEP1.5] wrote: {json_path}")
    return json_path

def run_step2(step1_html_path: Path, plan_json_path: Path, out_dir: Path, step2_prompt: str) -> Path:
    from ppt_requirements_collector import CamelAIClient
    camel = CamelAIClient()

    step1_html = read_text(str(step1_html_path))
    plan_json = read_text(str(plan_json_path))

    user_input = (
        "Render a single-file HTML deck.\n\n"
        "=== step1_5_plan.json ===\n"
        f"{plan_json}\n\n"
        "=== step1_generated.html ===\n"
        f"{step1_html}\n"
    )
    print("[STEP2][LLM] rendering visual deck ...")
    resp = camel.get_response(user_input, system_prompt=step2_prompt).strip()

    out_path = out_dir / STEP2_OUT_HTML
    write_text(out_path, resp)
...
def main():
    ...
    run_step2(step1_html_with_imgs, plan_json_path, out_dir, step2_prompt)
