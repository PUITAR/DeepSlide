...
"""
Full pipeline runner: Step1 -> Step1.5 -> Step2 (image-aware)

Inputs:
- beamer.txt (your generated Beamer frames, no template inserted pages)
- rendered.pdf (final PDF rendered with template; may include EXTRA pages)
- step1_prompt (for Beamer->HTML skeleton extraction)
- step1_template (for Step1 HTML wrapper)
- step1.5 prompt (planner)
- step2 prompt (renderer)
- keynote visual template HTML (style reference for step2 prompt)
...
Notes:
- Step1 uses DP monotonic alignment to map frames -> PDF pages (skipping inserted template pages).
- Snapshots are cropped from mapped PDF pages and embedded as base64 in Step1 HTML.
- Step1.5 MUST plan snapshot placement via diagram_spec(mode="snapshots").
- Step2 MUST copy snapshot images from step1_generated.html into final deck.
"""
...
STEP1_5_PROMPT_PATH = str(BASE_DIR / "html_diedai/step1_5_planner_prompt_v11_strictjson_img.txt")
STEP2_PROMPT_PATH   = str(BASE_DIR / "html_diedai/step2_executor_prompt_v10_img.txt")
...
SYSTEM_PROMPT_STEP15 = "You output ONLY valid JSON. No markdown. No commentary."
...
def _strip_json_to_object(text: str) -> str:
...
def _basic_json_repairs(cand: str) -> str:
...
def extract_json_obj(text: str) -> dict:
    cand = _strip_json_to_object(text)
    try:
        return json.loads(cand)
    except json.JSONDecodeError:
        cand2 = _basic_json_repairs(cand)
        return json.loads(cand2)

def fix_json_with_llm(client: CamelAIClient, bad_text: str, hint: str = "") -> dict:
    bad = bad_text
    if "{" in bad_text and "}" in bad_text:
        try:
            bad = _strip_json_to_object(bad_text)
        except Exception:
            bad = bad_text
    prompt = (
        "You are a JSON repair tool. Return STRICT valid JSON only. "
        "No commentary, no markdown, no code fences, no trailing commas, no comments.\n"
        + (f"Hint: {hint}\n" if hint else "")
        + "Fix the following JSON-like content:\n"
        + bad
    )
...
def run_step1_5(step1_html_path: Path, out_json: Path, step1_5_prompt_path: Path) -> dict:
    client = CamelAIClient()
    ...
    raw_path = out_json.with_suffix(out_json.suffix + ".raw.txt")
    raw_path.write_text(resp, encoding="utf-8")

    # Parse JSON; if invalid, ask LLM to repair into strict JSON
    try:
        obj = extract_json_obj(resp)
    except Exception as e:
        print("[STEP1.5][WARN] JSON parse failed, trying LLM repair:", repr(e))
        obj = fix_json_with_llm(client, resp, hint="Ensure commas/quotes are correct; remove trailing commas and any comments.")
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(obj, ensure_ascii=False, indent=2), encoding="utf-8")
    print("[STEP1.5] wrote:", out_json)
    return obj


def run_step2(step1_html_path: Path, plan_json_path: Path, out_html: Path,
             step2_prompt_path: Path) -> str:
    client = CamelAIClient()
    step1_html = step1_html_path.read_text(encoding="utf-8", errors="ignore")
    plan_json = plan_json_path.read_text(encoding="utf-8", errors="ignore")
    step2_prompt = step2_prompt_path.read_text(encoding="utf-8", errors="ignore")

    prompt = (
        step2_prompt
        + "\n\n-----BEGIN step1_5_plan.json-----\n" + plan_json + "\n-----END step1_5_plan.json-----\n"
        + "\n\n-----BEGIN step1_generated.html-----\n" + step1_html + "\n-----END step1_generated.html-----\n"
        + "\n\n-----BEGIN step1_template_keynote.html (visual reference)-----\n" + keynote_tpl + "\n-----END step1_template_keynote.html-----\n"
    )
    ...
    # keep only the outer HTML if model accidentally outputs extra
...
def main() -> None:
    ...
    plan_out  = out_dir / STEP1_5_OUT_JSON
    ...
    run_step2(step1_out, plan_out, step2_out, step2_prompt, step2_prompt)
