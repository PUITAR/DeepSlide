...
from visuals import _build_focus_zoom_widget, _build_table_viz_widget, _build_keynote_text_widget, _build_html_slide_section
from ppt_agent import _update_pdf_preview
from compiler import Compiler
import json

def generate_html_via_llm_iterative(tex_content, speeches, pdf_pages, focus_candidate_indices=None, per_slide_max_regions=None, enrich_effects=None):
    ...
    for i in range(0, len(speeches)):
        ...
        if current_frame_idx < len(matches):
            ...
            if should_try_focus:
                effects = enrich_effects
                if not effects:
                    effects = ["Image Focus", "Table Viz"]
                enable_image = "Image Focus" in effects
                enable_table = "Table Viz" in effects
                enable_text = "Text Keynote" in effects

                table_widget = ""
                if enable_table and has_tabular:
                    table_widget = _build_table_viz_widget(target_frame_latex)
                ...
                if enable_image and has_image:
                    ...
                    if not focus_src_bytes_or_path:
                        focus_log = "<h3>Log: Image detected but cannot be resolved.</h3>"
                    else:
                        focus_img_src = ""
                        if best_img_path:
                        ...
                        if focus_img_src and regions:
                            ...
                            focus_widget = _build_focus_zoom_widget(focus_img_src, regions, focus_interval_ms, focus_scale)
                        else:
                            print(f"DEBUG: Slide {i+1} - Image found but no regions generated.")
                            focus_log = "<h3>Log: AI could not identify focus regions for this image.</h3>"
                else:
                    if not table_widget:
                        focus_log = "<h3>Log: No image detected on this slide.</h3>"

                text_widget = ""
                if enable_text:
                    text_widget = _build_keynote_text_widget(target_frame_latex, speech_text)

                items = []
                if table_widget:
                    items.append(("Table", "table", table_widget))
                if focus_widget:
                    items.append(("Image", "image", focus_widget))
                if text_widget:
                    items.append(("Text", "text", text_widget))

                body = ""
                if len(items) >= 2:
                    default_tab = "table"
                    if items[0][1] != "table":
                        default_tab = items[0][1]
                    btns = "".join([
                        f"<button class=\"multi-tab{' is-active' if j==0 else ''}\" data-target=\"{name}\" type=\"button\">{label}</button>"
                        for j, (label, name, _) in enumerate(items)
                    ])
                    panes = "".join([
                        f"<div class=\"multi-pane{' is-active' if j==0 else ''}\" data-pane=\"{name}\">{html}</div>"
                        for j, (_, name, html) in enumerate(items)
                    ])
                    body = (
                        f"<div class=\"multi-widget\" data-default=\"{default_tab}\">"
                        f"<div class=\"multi-tabs\">{btns}</div>"
                        f"{panes}"
                        "</div>"
                    )
                elif len(items) == 1:
                ...
                if body:
                    rendered_html_sec = _build_html_slide_section(body, html_speech_text, slide_class="beamerish", index=i + 1)
    ...
    window.TableViz = (() => {
        ...
        function rgbaToSolid(rgba, fallback) {
            if (!rgba) return fallback;
            const m = String(rgba).match(/rgba\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)/i);
            if (!m) return rgba;
        ...
        function drawChart(canvas) {
            ...
            if (!cfg || !cfg.data || !cfg.data.labels || !cfg.data.datasets) return;
            ...
            clear(ctx, w, h);
            const pad = 40;
            const plot = { x0: pad, y0: pad * 0.6, x1: w - pad * 0.6, y1: h - pad };
            drawAxes(ctx, plot);
            ...
            const series = datasets.map(ds => {
                const vals = (ds.data || []).map(v => (v == null ? null : Number(v)));
                return { label: ds.label || '', data: vals, color: rgbaToSolid(ds.borderColor || ds.backgroundColor, '#3b82f6') };
            });
            ...
            const xStep = (plot.x1 - plot.x0) / n;
            const yScale = (plot.y1 - plot.y0) / (ymax - ymin);
            const xAt = (i) => plot.x0 + xStep * (i + 0.5);
            const yAt = (v) => plot.y1 - (v - ymin) * yScale;
            ...
            if (String(cfg.type || '').toLowerCase() === 'bar' && series.length >= 1) {
                const s0 = series[0];
                const barW = Math.max(8, Math.min(48, xStep * 0.7));
                ctx.fillStyle = rgbaToSolid(datasets[0].backgroundColor, s0.color);
                for (let i = 0; i < n; i++) {
                    const v = s0.data[i];
                    if (v == null || !Number.isFinite(v)) continue;
                    const x = xAt(i) - barW / 2;
                    const y = yAt(v);
                    ctx.fillRect(x, y, barW, plot.y1 - y);
                }
            } else {
                series.forEach((s, si) => {
        ...
        function setupSearch(root) {
            ...
            const table = root.querySelector('table.interactive-table');
        ...
        function setupSort(root) {
            ...
            const state = { idx: -1, asc: true };
            ths.forEach((th, idx) => {
                th.addEventListener('click', () => {
                    state.asc = (state.idx === idx) ? !state.asc : true;
                    state.idx = idx;
                    ths.forEach(t => t.classList.remove('sort-asc', 'sort-desc'));
                    th.classList.add(state.asc ? 'sort-asc' : 'sort-desc');
                    const rows = Array.from(tbody.querySelectorAll('tr'));
                    rows.sort((a, b) => {
                        const ca = a.children[idx]?.textContent || '';
                        const cb = b.children[idx]?.textContent || '';
                        const na = parseMaybeNumber(ca);
                        const nb = parseMaybeNumber(cb);
                        let cmp;
        ...
        function build(root) {
            function start() {
                if (root.__tv_inited) return;
                root.__tv_inited = true;
                setupSearch(root);
                setupSort(root);
                root.querySelectorAll('canvas.auto-chart').forEach(drawChart);
            }
        ...
        function checkActive() {
            controllers.forEach((ctl, el) => {
                if (el.closest('section.active')) ctl.start();
            });
        }

        function initAll() {
            document.querySelectorAll('.table-viz').forEach(el => {
                if (!controllers.has(el)) controllers.set(el, build(el));
            });
            window.addEventListener('resize', () => {
                controllers.forEach((ctl, el) => {
                    if (el.closest('section.active')) ctl.redraw();
                });
            });
            checkActive();
        }
    ...
    window.MultiWidget = (() => {
        function initAll() {
            document.querySelectorAll('.multi-widget').forEach(w => {
                if (w.__mw_inited) return;
                w.__mw_inited = true;
                const tabs = Array.from(w.querySelectorAll('.multi-tab'));
                const panes = Array.from(w.querySelectorAll('.multi-pane'));
                const defaultName = w.getAttribute('data-default') || (tabs[0]?.getAttribute('data-target') || '');
                const activate = (name) => {
                    tabs.forEach(t => t.classList.toggle('is-active', t.getAttribute('data-target') === name));
                    panes.forEach(p => p.classList.toggle('is-active', p.getAttribute('data-pane') === name));
                    if (window.FocusZoom) window.FocusZoom.checkActive();
                    if (window.TableViz) window.TableViz.checkActive();
                };
                tabs.forEach(t => {
                    t.addEventListener('click', () => activate(t.getAttribute('data-target')));
                });
                if (defaultName) activate(defaultName);
            });
        }
        return { initAll };
    })();
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .multi-pane {{ display: none; }}
        .multi-pane.is-active {{ display: block; }}
        ...
        table.interactive-table thead th {{
            position: sticky;
            top: 0;
            z-index: 2;
            text-align: left;
        ...
        table.interactive-table thead th.sort-asc::after {{
            content: " ▲";
            color: rgba(59,130,246,0.9);
            font-size: 12px;
        }}
        table.interactive-table thead th.sort-desc::after {{
            content: " ▼";
            color: rgba(59,130,246,0.9);
            font-size: 12px;
        }}
        table.interactive-table tbody td {{
            padding: 10px 12px;
        ...
        .keynote {{
            ...
            border-radius: 22px;
            background:
                radial-gradient(900px 360px at 12% 10%, rgba(59,130,246,0.18), transparent 60%),
                radial-gradient(800px 360px at 88% 18%, rgba(34,197,94,0.14), transparent 55%),
        ...
        .keynote::before {{
            ...
            position: absolute;
            inset: -2px;
            background: linear-gradient(120deg, rgba(59,130,246,0.22), rgba(34,197,94,0.18), rgba(99,102,241,0.18));
            opacity: 0.22;
            filter: blur(18px);
            pointer-events: none;
        }}
...
<body>
    ...
    <script>
        ...
        window.onload = () => {{
            ...
            if(window.TableViz) window.TableViz.initAll();
            if(window.MultiWidget) window.MultiWidget.initAll();
        }};
    </script>
</body>
</html>
