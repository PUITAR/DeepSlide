...
"""
    ...
    def _fix_content(self, base_dir: str, error_message: str, obj_idx: int = None) -> bool:
        ...
        try:
            ...
            prompt_suffix = ""
            if "not found" in error_message.lower() or "unable to load" in error_message.lower():
                prompt_suffix = f"\n\nHint: It seems like a file is missing. You can use tools to `list_files` in `{base_dir}` or `find_file` to locate it in parent directories (e.g. `/app/tmp_uploads`), and then `copy_file` it to the correct location expected by the LaTeX code."
