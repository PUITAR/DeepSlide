...
def main() -> int:
    ...
    repo_root = role_root.parent.parent

    load_dotenv(repo_root / "deepslide-v4" / ".env")
    load_dotenv(repo_root / "deepslide" / ".env")
    load_dotenv(role_root / ".env")

    def run_step(args):
        return subprocess.run([sys.executable, "run_eval.py", *args], cwd=str(role_root), env=os.environ.copy()).returncode
