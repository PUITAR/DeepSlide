...
class LogicChainAgent:
    """Logicchain adapted for plain-text input.

    Public API:
    - generate_options(raw_text, audience_profile) -> LogicChainOptions

    Notes:
    - Configuration is loaded via combine124.config.load_env.
    """
    ...
    def __init__(
        ...
        self.model_url = model_url or os.getenv("DEFAULT_MODEL_API_URL") or "https://api.deepseek.com"
        effective_model_type = model_type or os.getenv("DEFAULT_MODEL_TYPE") or "deepseek-chat"

        api_key = (
            os.getenv(api_key_env)
            or os.getenv("DEFAULT_MODEL_API_KEY")
    ...
    def generate_options(
        ...
        if not raw_text.strip():
            raise RuntimeError("generate_options: raw_text is empty")
    ...
    def _call_llm_select_templates(
        ...
        resp = self.selector_agent.step(BaseMessage.make_user_message(role_name="User", content=user_content))
    ...
    def _generate_with_validation(
        ...
        resp = self.chain_agent.step(BaseMessage.make_user_message(role_name="User", content=user_content))
        ...
        issues = self._validate_chain_data(data, template)
        if not issues:
            return data

        repair_prompt = self._build_repair_prompt(content, issues)
        repair_resp = self.chain_agent.step(BaseMessage.make_user_message(role_name="User", content=repair_prompt))
        ...
        repair_issues = self._validate_chain_data(repaired, template)
    ...
    def _build_repair_prompt(self, bad_json_text: str, issues: List[str]) -> str:
    ...
    def _validate_chain_data(self, data: Dict[str, Any], template: NarrativeTemplate) -> List[str]:
        issues: List[str] = []
