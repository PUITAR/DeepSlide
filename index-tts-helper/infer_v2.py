...
USE_MODELSCOPE = os.getenv('USE_MODELSCOPE', '1').lower() in ('true', '1', 't')
...
class IndexTTS2:
    def __init__(
        ...
        if USE_MODELSCOPE:
            try:
                w2v_dir = snapshot_download('facebook/w2v-bert-2.0', revision='master')
                self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(w2v_dir)
                print(f">> Load w2v-bert from: {w2v_dir}")
            except Exception as e:
                st.error(f">> Load w2v-bert failed: {e}")
        else:
            self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained("facebook/w2v-bert-2.0")

        print(">> semantic_model weights restored from:", os.path.join(self.model_dir, self.cfg.w2v_stat))
        ...
        if USE_MODELSCOPE:
            semantic_code_ckpt = snapshot_download("amphion/MaskGCT")
            semantic_code_ckpt = os.path.join(semantic_code_ckpt, "semantic_codec/model.safetensors")
        else:
            semantic_code_ckpt = hf_hub_download("amphion/MaskGCT", filename="semantic_codec/model.safetensors")

        print(f">> Load semantic_codec from: {semantic_code_ckpt}")
        ...
        # load campplus_model
        # if USE_MODELSCOPE:
        #     campplus_ckpt_path = snapshot_download("funasr/campplus")
        #     campplus_ckpt_path = os.path.join(campplus_ckpt_path, "campplus_cn_common.bin")
        # else:
        #     campplus_ckpt_path = hf_hub_download("funasr/campplus", filename="campplus_cn_common.bin")
        campplus_ckpt_path = hf_hub_download("funasr/campplus", filename="campplus_cn_common.bin")

        print(f">> Load campplus_model from: {campplus_ckpt_path}")
        ...
        if USE_MODELSCOPE and bigvgan_name == "nvidia/bigvgan_v2_22khz_80band_256x":
            redirect_modelscope_name = "nv-community/bigvgan_v2_22khz_80band_256x"
            bigvgan_name = snapshot_download(redirect_modelscope_name)
